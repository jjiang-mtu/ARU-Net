...
if os.path.exists(para.test_set_path):
    shutil.rmtree(para.test_set_path)

new_ct_path = os.path.join(para.test_set_path, 'ct')
new_seg_dir = os.path.join(para.test_set_path, 'seg')

os.mkdir(para.test_set_path)
...
for file in tqdm(os.listdir(para.test_ct_path)):
    ...
    ct = sitk.ReadImage(os.path.join(para.test_ct_path, file), sitk.sitkUInt16)
    ct_array = sitk.GetArrayFromImage(ct)

    seg = sitk.ReadImage(os.path.join(para.test_seg_path, file.replace('volume', 'segmentation')), sitk.sitkInt8)
